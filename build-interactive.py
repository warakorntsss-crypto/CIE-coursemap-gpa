...
#summary{position:fixed; left:14px; bottom:14px; z-index:35; width:248px; max-width:80vw;
...
.cyrow{display:flex; justify-content:space-between; gap:6px; padding:2px 6px; border-top:1px solid #eef2f7}
.cyrow span{font-family:"IBM Plex Mono",monospace}
.cyrow b{font-weight:700; min-width:22px; text-align:right}
...
@media (max-width:720px){
  ...
  .node .star{padding:8px; font-size:17px}
}
</style>
</head>
<body>
...
DATA.sems.forEach((s,i)=>{
  ...
  h.innerHTML="<b>"+s.label+"</b><span>"+s.cr+" Credit</span>"; colsBar.appendChild(h);
});
...
function buildDoc(mode){
  ...
  document.getElementById('docPaper').innerHTML = docMode==='compact'?buildCompact(today):buildFull(today);
